...
function buildExtractorTools(store) {
	return [
		...
		defineTool({
			...
			description: "配置会话提取：enabled（总开关）、intervalMinutes（每隔多少分钟自动提取一次，0=关闭）、windowDays（扫描最近几天会话）、maxCandidatesPerRun（每次最多候选数）、skillLanguage（正文语言 zh/en）、llmBaseUrl/llmApiKey/llmModel（LLM，OpenAI 兼容）、targetDir（Obsidian skill 镜像目录，默认 2️⃣ AI/Skill）、agentSkillDir（agent 可自动加载的 skill 根，默认 ~/.agents/skills；接受后会双写：agent 根 + Obsidian 镜像）。持久化到 ~/.dsh/dsh-skill-studio/extractor.json（0600）。传 reset: true 恢复默认。不会泄露 llmApiKey。",
			parameters: {
				...
				targetDir: { type: "string", description: "Obsidian skill 镜像目录" },
				agentSkillDir: { type: "string", description: "agent 自动加载的 skill 根（默认 ~/.agents/skills）" },
			...
			async execute(args) {
				try {
					...
					return {
						ok: true,
						message: "配置已保存：" + (view.enabled ? "启用" : "禁用") + " · " + (view.intervalMinutes === 0 ? "已关闭定时" : "每 " + view.intervalMinutes + " 分钟") + " · 窗口 " + view.windowDays + " 天 · 正文" + (view.skillLanguage === "en" ? "英文" : "中文") + " · LLM " + (view.configured ? "已配置" : "未配置") + " · 双写 agent根+Obsidian",
						enabled: view.enabled,
						intervalMinutes: view.intervalMinutes,
						windowDays: view.windowDays,
						llmConfigured: view.configured,
			...
			async execute(args) {
				try {
					...
					for (const c of queue) {
						if (ids.has(c.id)) {
							const r = await writeSkillDual(cfg.agentSkillDir, cfg.targetDir, c);
