...
function buildExtractorTools(store) {
	return [
				...
				try {
					...
					return {
						...
						message:
							...
							" · 默认存 agent 根（" + view.agentSkillDir + "）" + (view.mirrorDir ? " · 额外镜像 " + view.mirrorDir : ""),
						...
						mirrorDir: view.mirrorDir,
						agentSkillDir: view.agentSkillDir,
		...
		defineTool({
			...
			description: "配置会话提取：enabled（总开关）、intervalMinutes（每隔多少分钟自动提取一次，0=关闭）、windowDays（扫描最近几天会话）、maxCandidatesPerRun（每次最多候选数）、skillLanguage（正文语言 zh/en）、llmBaseUrl/llmApiKey/llmModel（LLM，OpenAI 兼容）、mirrorDir（可选额外镜像目录，默认关闭；留空=只存 agent 根）、agentSkillDir（agent 自动加载根，默认 ~/.agents/skills，**必须、默认、不用设置**）。接受默认写入 agent 根（让 DSH 自动引用），只有设置了 mirrorDir 才额外保存第二份。持久化到 ~/.dsh/dsh-skill-studio/extractor.json（0600）。传 reset: true 恢复默认。不会泄露 llmApiKey。",
			parameters: {
				...
				mirrorDir: { type: "string", description: "可选额外镜像目录（默认关闭，留空=只存 agent 根）" },
				agentSkillDir: { type: "string", description: "agent 自动加载根（默认 ~/.agents/skills，必须，不用设置）" },
			...
			async execute(args) {
				try {
					...
					return {
						ok: true,
						message: "配置已保存：" + (view.enabled ? "启用" : "禁用") + " · " + (view.intervalMinutes === 0 ? "已关闭定时" : "每 " + view.intervalMinutes + " 分钟") + " · 窗口 " + view.windowDays + " 天 · 正文" + (view.skillLanguage === "en" ? "英文" : "中文") + " · LLM " + (view.configured ? "已配置" : "未配置") + " · 默认存 agent 根" + (view.mirrorDir ? " · 额外镜像 " + view.mirrorDir : ""),
						enabled: view.enabled,
						intervalMinutes: view.intervalMinutes,
						windowDays: view.windowDays,
						llmConfigured: view.configured,
			...
			async execute(args) {
				try {
					...
					for (const c of queue) {
						if (ids.has(c.id)) {
							const r = await writeSkillDual(cfg.agentSkillDir, cfg.mirrorDir, c);
							if (r.written && r.written.length) written.push(c.name);
							else kept.push(c);
						} else kept.push(c);
					}
...
export { ExtractorStore, assembleCandidateFile, defaults as extractorDefaults, loadCandidates, runExtraction, saveCandidates, updateCandidate, writeSkill, writeSkillDual } from "./extractor.js";
