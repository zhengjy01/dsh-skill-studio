...
/** Agent skill root that DSH's model-facing registry actually loads (user-agents). */
export const DEFAULT_AGENT_SKILL_DIR = join(homedir(), ".agents", "skills");
...
export function defaults() {
	return {
		...
		agentSkillDir: DEFAULT_AGENT_SKILL_DIR,
		lastRunAt: "",
		lastRunSummary: ""
	};
}
...
/**
 * Write a candidate skill to BOTH the agent skill root (so DSH's model-facing
 * registry can load it) and the Obsidian mirror (for the knowledge base). Each
 * write dedups against its own dir; the result is ok when at least one wrote.
 */
export async function writeSkillDual(agentDir, obsidianDir, cand, { force = false } = {}) {
	const results = [];
	for (const dir of [agentDir, obsidianDir]) {
		if (!dir) continue;
		results.push({ dir, ...(await writeSkill(dir, cand, { force })) });
	}
...
export async function runExtraction(cfg, root = sessionsRoot(), { now = Date.now() } = {}) {
	...
	const existing = new Set([
		...
		...await existingSkillNames(cfg.targetDir)
	]);
	const queue = await loadCandidates();
