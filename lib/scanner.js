...
/** Default Obsidian personal skill library (registered as a DSH custom source). */
const DEFAULT_OBSIDIAN_SKILL_DIR = "/Users/zhengjunyao/Documents/Obsidian Vault/2️⃣ AI/Skill";
/** Custom skill dirs to scan on top of the standard roots (env-separated by ":"). */
function customSkillDirs() {
	const o = process.env.DSH_SKILL_MANAGER_SKILL_DIRS;
	return typeof o === "string" && o !== "" ? o.split(":") : [DEFAULT_OBSIDIAN_SKILL_DIR];
}
...
export async function skillRoots(cwd) {
	...
	// Custom personal skill library (Obsidian) — registered as a DSH custom
	// skill source, so the panel should also see and edit those skills.
