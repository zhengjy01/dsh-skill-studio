		...
		function ExtractorPanel() {
			...
			const status = view
				? "插件" + (view.enabled ? "已启用" : "已禁用") +
					...
					" · 默认存 agent 根" + (view.mirrorDir ? " · 额外镜像 " + view.mirrorDir : "（不存镜像）")
				: "加载中…";
			return h(
				"div",
				...
				[
					...
					h("div", { style: s.status, key: "agentinfo" }, "保存到 agent 自动加载根： " + (agentSkillDir || "~/.agents/skills") + "（固定，DSH 自动引用）"),
					h("input", { style: smallInput({ width: "100%" }), key: "mirror", placeholder: "额外镜像目录（可选，留空=只存 agent 根）", value: mirrorDir, onChange: (e) => setMirrorDir(e.target.value) }),
