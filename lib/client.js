window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ExtractorPanel() {
			...
			const [agentSkillDir, setAgentSkillDir] = React.useState("");
			...
			const load = React.useCallback(async () => {
				try {
					...
					setAgentSkillDir(v.agentSkillDir || "");
			...
			const saveConfig = () =>
				run(async () => {
					const patch = {
						...
						agentSkillDir: agentSkillDir.trim()
					};
			...
			return h(
				...
				[
					...
					h("input", { style: smallInput({ width: "100%" }), key: "dir", placeholder: "Obsidian 镜像目录（2️⃣ AI/Skill）", value: targetDir, onChange: (e) => setTargetDir(e.target.value) }),
					h("input", { style: smallInput({ width: "100%" }), key: "agentdir", placeholder: "agent 自动加载根（~/.agents/skills）", value: agentSkillDir, onChange: (e) => setAgentSkillDir(e.target.value) }),
